        ...
        while True:
            ...
            try:
                if next_state == fsm.STATE_PRE_POUR:
                    ...
                    log.error("Current state: %d, event %d. Can't find next state." % (
                        cur_state, event))
                    raise BartendroBrokenError(
        ...
        for booze_id in sorted(self.recipe.data.keys()):
            ...
            for i in range(self.disp_count):
                ...
                if booze_id == disp.booze_id:
                    ...
                    if ml > MAX_DISPENSE:
                        raise BartendroCantPourError(
                            "Cannot make drink. Invalid dispense quantity: %d ml. (Max %d ml)" % (ml, MAX_DISPENSE))
            ...
            if not found:
                raise BartendroCantPourError(
                    "Cannot make drink. I don't have the required booze: %d" % booze_id)
    ...
    def _check_liquid_levels(self):
        ...
        if not self.driver.update_liquid_levels():
            raise BartendroLiquidLevelReadError(
                "Failed to update liquid levels")
        ...
        for disp in active_disp:
            while True:
                ...
                if is_dispensing < 0 or over_current < 0:
                    log.error(
                        "Is dispensing test on dispenser %d failed. Ignoring." % (disp + 1))
                    sleep(.2)
                    continue
